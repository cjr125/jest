...
const {performance} = require('perf_hooks'); // eslint-disable-next-line import/no-extraneous-dependencies

const workerFarm = require('worker-farm');

const JestWorker = require('../../build').Worker; // Typical tests: node --expose-gc test.js empty 100000
...
function testWorkerFarm() {
  return new Promise(async resolve => {
    ...
    async function countToFinish() {
      if (++count === calls) {
        workerFarm.end(api);
        const endTime = performance.now(); // Let all workers go down.
...
function testJestWorker() {
  return new Promise(async resolve => {
    ...
    farm.getStderr().pipe(process.stderr); // Let all workers come up.
...
async function main() {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    await sleep(3000); // eslint-disable-next-line no-undef
  ...
  console.log('-'.repeat(75));
  console.log('total worker-farm:', {
    wFGT,
    wFPT
  });
  console.log('total jest-worker:', {
    jWGT,
    jWPT
  });
  console.log('-'.repeat(75));
