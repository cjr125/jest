...
class ScriptTransformer {
  constructor(config, cacheFS = new Map()) {
  ...
  _getFileCachePath(filename, content, options) {
    ...
    const cacheKey = this._getCacheKey(content, filename, options); // Create sub folders based on the cacheKey to avoid creating one
  ...
  _getTransformer(filename) {
    ...
    let transformer = require(transformPath);
  ...
  transformSource(filepath, content, options) {
    ...
    const shouldCallTransform = transformer && this.shouldTransform(filename); // That means that the transform has a custom instrumentation
